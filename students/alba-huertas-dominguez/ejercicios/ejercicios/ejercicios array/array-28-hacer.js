...
const arr=["lunes", "martes","miercoles"]

//Imprimir con un P si hay alguna palabra con más de una letra 't
// necesitamos el some 

//forma 1.
// voy a hacer una funcion para evaluar si las palabras tienen o no t.
function mast(str){
...
//forma 2.
// utilizo el split, que es partir para hacer un array, si le pongo la t, lo separo cuando encuentre t. si quiero buscar 2 t el array tiene que tener 3 elementos.
document.write(`${arr.some(v =>v.toLocaleLowerCase().split('t').length>3)}`);

//forma 3. expresiones regulares:
document.write(`${arr.some(/.*t.*t/i.test(v))}`)
...
const allword = arr.every((v,i,array) => v.toLocaleLowerCase().includes('a'));
document.write(allword)

//version REGEX:

arr.every(v=> /.*a/i.test(w));
document.write(arr);


// imprimir con una p la primera palabra que empiece por h o el mensaje no hay palabras que empiecen por h.
// con el find busco la primera cosa que cumpla la condicion, con el starstwith analizo la primera letra de la palabra. entonces aqui le estoy diciendo
// que me busque en el valor,en minuscula, la primera palabra que empiece por h. 

arr.find(v=> v.toLowerCase().startsWith('h'));
...
arr.map(v =>reverseStr(v));

// imprimir con un p cuantas palabras son infinitivos (terminan en ar er o ir ).
// hay que utilizar el reduce
...
const countryJ = countryListAllIsoData.find(v => v.name.startsWith('J'));

document.write(countryJ.name)
