...
document.write(`<h1>Countries starting with 'J':</h1>`)
countryListAllIsoData.forEach(c => {
    if (c.name.toLowerCase().startsWith('j')) document.write(`<li>${c.name}</li>`)
})

/* country with MG code */
const countryWithMGCode = countryListAllIsoData.find(c => c.code === 'MG')
document.write(`<h1>Country with 'MG' code is ${countryWithMGCode.name}</h1>`)

/* find country with more letters in name */
const listNumberOflETTERS = countryListAllIsoData.map(c => c.name.length)
const countryWithLongestName = (Math.max(...listNumberOflETTERS))
const indexOfCountryWithLongestName = listNumberOflETTERS.indexOf(countryWithLongestName)
document.write(`<h1>Country with longest name is ${countryListAllIsoData[indexOfCountryWithLongestName].name}</h1>`)

/* print all countries ending with s */
document.write(`<h1>Countries starting with 'S':</h1>`)
const countriesObjectEndingWithS = countryListAllIsoData.filter(c => c.name.toLowerCase().endsWith('s'))
const namesOfCountriesWithS = countriesObjectEndingWithS.map(c => c.name)
namesOfCountriesWithS.forEach(c => document.write(`<li>${c}</li>`))
